  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FavouriteCubit,FavouritesStates>(
      builder: (context, state) {
        var fCubit = BlocProvider.of<FavouriteCubit>(context)..fetchFavouriteBooks();
        return Scaffold(
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => CustomRecommendedItem(
                      authorName: fCubit.favouriteBooks[index]['volumeInfo']['authors']?[0] ?? '',
                      bookName: fCubit.favouriteBooks[index]['volumeInfo']['title'],
                      description: fCubit.favouriteBooks[index]['volumeInfo']['description'] ?? fCubit.favouriteBooks[index]['volumeInfo']['infoLink'],
                      bookImage: fCubit.favouriteBooks[index]['volumeInfo']['imageLinks']?['smallThumbnail'] ?? '',
                    ),
                  ),
                ),
              ],
